...
public:
    ...
    //initialization
    Node(T k, K v, Color c, Node *p, Node *l, Node *r):
...
void RBTree<T,K>::insertFixUp(Node<T,K>* &root, Node<T,K> *node){
    ...
    while(node->parent!=NULL && node->parent->color==RED) {  // if parent is red
        ...
        if(node->parent->parent->left==node->parent) {          // if inserted node is the left child of its parent
            Node<T,K> *uncle=node->parent->parent->right;       // set uncle node
            ...
            Node<T,K> *uncle=node->parent->parent->left;
...
*/
template <class T,class K>
void RBTree<T,K>::insert(T key, K value){
    Node<T,K> *x=NULL;
    if ((x=new Node<T,K>(key,value,RED,NULL,NULL,NULL))==NULL){
...
void RBTree<T,K>::print(Node<T,K>* tree, T key, int direction){
    if (tree!=NULL){
        if (direction==0)//is root node
            ...
            std::cout<<tree->key<<((tree->color==RED)?"(R)":"(B)")<<" is "<< key <<"'s "<<(direction==1?"right child" : "left child")<<", value="<<tree->value<<endl;
